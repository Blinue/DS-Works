...
// 移动操作
	_SeqList_iterator &operator++() {
		assert((_SeqList_const_iterator<Ty, Alloc>::_verify_valid(_SeqList_const_iterator<Ty, Alloc>::_cur + 1)));	// 非尾后迭代器
	...
	_SeqList_iterator &operator--() {
		assert((_SeqList_const_iterator<Ty, Alloc>::_verify_valid(_SeqList_const_iterator<Ty, Alloc>::_cur - 1)));	// 非首元素迭代器
