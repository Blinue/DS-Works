...
struct _seq_list_const_iterator
{
    ...
    [[nodiscard]] const Ty &operator[](difference_type off) const
    {
        assert(_verify_valid(_seq_list_const_iterator<Ty, Alloc>::_cur + off));

        return *(_seq_list_const_iterator<Ty, Alloc>::_cur + off);
    }
...
struct _seq_list_iterator : public _seq_list_const_iterator<Ty, Alloc>
{
    ...
    [[nodiscard]] Ty &operator[](difference_type off)
    {
        assert((_seq_list_const_iterator<Ty, Alloc>::_verify_valid(_seq_list_const_iterator<Ty, Alloc>::_cur + off)));
...
public:
    ...
    {
        if (!_valid_iterator(first) || !_valid_iterator(last) || first > last)
        {
            throw new std::exception("不合法的迭代器");
        }
    ...
    // 与 other 交换内容
    void swap(seq_list &other) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<allocator_type>::is_always_equal::value)
    {
